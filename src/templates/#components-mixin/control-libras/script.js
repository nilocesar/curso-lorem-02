...
events.on('ready', function () {
    searchDataLibras = function (_view) {
        view = _view;
    ...
    initLibras = function (configObj) {
        configLibras = configObj;
    }

    callVimeoData = async function () {
        if (configLibras.vimeoAPICall) {
            ...
            $.getJSON(`${origin}/app/data/libras.json`, function (data) {
    ...
    setActiveLibrasBt = function (btnLibras) {
        btnLibras.click(function () {
            activeLibras();
        });
    }

    activeLibras = function () {
        ...
        if (arrVideosLibras.length == 0) {
            var lastCalled;
            var newHtml = `<img style='width:4em; margin-right:1em;' src='${origin}/app/assets/img/hand_anima.gif'/></div><div>Aguarde, carregando Libras...</div><div>`;
    ...
    findVimeoEmbedByName = function (videoName) {
    ...
    setLibrasVideo = function (html_video) {
        $(".librasDiv").find('.video').empty();
        ...
        if (arrVideosLibras.length > 0 && scorm.loadObject("librasActive") == true) {
            ...
            if (type == "vimeo") {
                ...
                if (videoID == undefined) {
                    newHtml = `<img style='width:4em; margin-right:1em;' src='${origin}/app/assets/img/hand_anima.gif'/></div><div>Vídeo não encontrado.</div>`
                }

            } else if (type == "mp4") {
                newHtml = `<video width="100%" height="100%" controls><source src="${origin}/app/assets/videos/${url}.mp4" type="video/mp4"></video>`;

            } else if (type == "gif") {
                newHtml = `<img src="${origin}/app/assets/img/${url}.gif"/>`;
